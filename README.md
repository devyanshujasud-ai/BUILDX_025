# 🕳️ Vikasit Nagpur (BUILD-X 2026) — Autonomous Road Defect Detection, Smart Budget Optimizer & Hardware IoT Platform
...
[![ESP32](https://img.shields.io/badge/Hardware-ESP32_Microcontroller-E7352C.svg?logo=espressif&logoColor=white)](https://www.espressif.com/)

An enterprise-grade, end-to-end intelligent road infrastructure monitoring, AI vision, IoT hardware telemetry, and smart budget allocation governance platform engineered for **Vikasit Nagpur (BUILD-X 2026)**. The system ingests visual road feeds (citizen uploads, dashcam footage, and live webcams), applies deep-learning computer vision to detect potholes, streams real-time physical telemetry from ESP32 vehicle scanners (ultrasonic distance & MFRC522 RFID contractor authorization), mathematically optimizes road maintenance under severe 40% municipal budget cuts via multi-criteria Knapsack algorithms, determines civic jurisdiction via GIS polygon geofencing, automatically dispatches municipal grievance tickets, and manages repair lifecycles through an interactive civic Kanban system.
...
3. [Deep-Dive Module Breakdown](#3-deep-dive-module-breakdown)
   ...
   - [3.9 Smart Budget Optimizer (Challenge 4: 40% Maintenance Budget Cut)](#39-smart-budget-optimizer-challenge-4-40-maintenance-budget-cut)
   - [3.10 Physical ESP32 Hardware & RFID Inspection Integration](#310-physical-esp32-hardware--rfid-inspection-integration)
...
### 3.9 Smart Budget Optimizer (Challenge 4: 40% Maintenance Budget Cut)
Located in `Frontend/src/components/SmartBudgetOptimizer.jsx` & `Backend/app/services/budget_service.py`:
- **The Civic Dilemma**: When city municipal corporations face an unexpected 40% fiscal austerity reduction, arbitrary cutbacks result in deteriorating arterial roads and increased fatal accidents.
- **Multi-Criteria Knapsack Optimization**:
  $$W_i = w_{\text{traffic}} \cdot T_i + w_{\text{accident}} \cdot A_i + w_{\text{complaints}} \cdot C_i + w_{\text{economic}} \cdot E_i$$
  $$\text{BCR}_i = \frac{W_i}{\text{Estimated Repair Cost}_i}$$
  Maximizes total public utility subject to the reduced capital constraint:
  $$\sum_{i \in \text{Allocated}} \text{Cost}_i \le B_{\text{reduced}} = B_{\text{baseline}} \times (1 - \text{reduction\_pct})$$
- **Interactive Simulation**: Dynamically adjust budget reduction percentages and criterion weights with real-time recalculated Benefit-Cost Ratios (BCR), corridor prioritization rankings, interactive GIS mapping of funded vs deferred corridors, and comprehensive audit defense justifications.
- **Quantified Impact Metrics**: Tracks preserved infrastructure quality, daily citizens safeguarded, accident blackspots mitigated, and emergency contingency reserves.

### 3.10 Physical ESP32 Hardware & RFID Inspection Integration
Located in `Hardware/` & `Frontend/src/components/HardwareMonitor.jsx`:
- **ESP32 Edge Microcontroller**:
  - **Ultrasonic Distance Sensor (HC-SR04)**: Measures road surface profile and depression depth at 10Hz; triggers instant defect telemetry when distance excursion exceeds safety thresholds.
  - **MFRC522 13.56MHz RFID Reader**: Physical smart card reader authenticating field maintenance contractors and civic inspectors on-site before initiating or closing repairs.
  - **WS2812 NeoPixel Status Ring**: Real-time visual feedback (Green = nominal scan, Yellow = warning/moderate defect, Red = severe pothole detected, Blue = authorized RFID card swipe).
  - **Wi-Fi Telemetry Reporter**: Streams live sensor telemetry, pothole event coordinates, and RFID audit taps directly to `/api/hardware/telemetry` over HTTP.
- **Python Hardware Simulator (`Hardware/esp32_simulator.py`)**:
  - Fully simulates ESP32 edge device with ultrasonic oscillations, realistic pothole detection events along Nagpur arterial routes, and randomized contractor RFID badge scans for headless testing without physical breadboards.
...
```
BUILD-X / Vikasit Nagpur/
│
├── Backend/
│   ├── app/
│   │   ├── __init__.py                  # Application package identifier
│   │   ├── main.py                      # FastAPI app with budget & hardware routers
...
│   │   │   └── budget_schema.py         # SmartBudget request & response schemas
...
│   │   │   └── budget_service.py        # Multi-criteria Knapsack budget optimization engine
...
│   │       ├── budget_routes.py         # /api/budget/corridors, /optimize, /summary
│   │       └── hardware_routes.py       # /api/hardware/telemetry, /status, /events
│   │
│   ├── uploads/                         # Stored original, annotated, and repair images
│   ├── run.py                           # Server launcher script (uvicorn on port 8000)
│   ├── test_backend.py                  # Full backend API & AI verification suite
│   ├── test_budget_optimization.py      # Unit tests for Knapsack budget optimization
...
│   ├── src/
│   │   ├── components/
│   │   │   ├── Navbar.jsx               # Navigation header with Budget & Hardware tabs
│   │   │   ├── DetectionStudio.jsx      # Multi-mode upload studio with Leaflet map
│   │   │   ├── TicketKanban.jsx         # Civic Kanban board with proof upload
│   │   │   ├── AnalyticsDashboard.jsx   # KPI charts & authority breakdown
│   │   │   ├── AuthorityDirectory.jsx   # Civic authority directory cards
│   │   │   ├── SmartBudgetOptimizer.jsx # 40% Budget cut optimizer & corridor matrix
│   │   │   ├── HardwareMonitor.jsx      # Real-time ESP32 telemetry & RFID tap monitor
│   │   │   ├── PotholeDetailModal.jsx   # Modal for deep defect inspection & PDF download
│   │   │   └── SettingsModal.jsx        # Database & AWS connection modal
│   │   ├── services/
│   │   │   └── api.js                   # Axios HTTP client with Budget & Hardware APIs
│   │   ├── i18n/
│   │   │   └── translations.js          # Trilingual translations (English, Hindi, Marathi)
│   │   ├── App.jsx                      # Primary layout coordinator & route state
...
│
├── Hardware/
│   ├── smart_city_wifi.ino              # ESP32 Arduino firmware (Ultrasonic + RFID + NeoPixel)
│   ├── smart_city_wifi/
│   │   └── smart_city_wifi.ino          # Arduino IDE compatible sketch directory
│   └── esp32_simulator.py               # Hardware telemetry & road scan simulator
│
...
### Run Smart Budget Optimization Suite
Verify the multi-criteria Knapsack algorithm, 40% budget cut scenarios, and edge conditions:

```bash
python Backend/test_budget_optimization.py
```

### Run ESP32 Physical Hardware Telemetry Simulator
Simulate road scanning vehicle with ultrasonic depth sensing and contractor RFID badges:
